...
void SetUIPlayerstatRedirect(const char* xml)
{
    if (!xml || xml[0] == '\0') {
        ...
        WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI playerstat redirect cleared");
        return;
    }
    ...
    WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI playerstat redirect configured");
}
...
const char* ResolveUIPlayerstatXml(const char* xml)
{
    if (xml &&
        ...
        g_playerstatRedirectXml[0] != '\0') {
        WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI redirecting playerstat window");
