...
void RequestUIDaychangeRedirect(const char* xml, DWORD ttlMs)
{
    ...
    WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI redirect armed");
}
...
const char* ResolveUIDaychangeXml(const char* xml, DWORD now)
{
    ...
    if (redirectActive && daychangeRequested) {
        WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI redirecting daychange window");
    ...
    if (daychangeRequested) {
        ...
        WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI observed vanilla daychange window");
    }

    if (g_messageRedirectUntil != 0 && !redirectActive) {
        WriteDebugLog(OYNONTOOLS_DEBUG_CHANNEL, "Oynon UI redirect expired before daychange window creation");
        g_messageRedirectUntil = 0;
        g_redirectXml[0] = '\0';
    }
